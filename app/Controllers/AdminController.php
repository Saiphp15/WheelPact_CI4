...
class AdminController extends BaseController {
    ...
    public function save_branch() {
        ...
        try {
            ...
            $validation->setRules([
                ...
                'branchName' => 'required',
                'branchType' => 'required',
                'branchSupportedVehicleType' => 'required',
    ...
    public function edit_branch_details($branchId) {
        ...
        return view('admin/edit_branch_details', $this->pageData);
    }
    ...
    public function edit_update_branch_details() {
        ...
        try {
            ...
            $file = $this->request->getFile('branchThumbnail');
            if ($file->isValid()) {
                /* upload Branch Thumbnail */
                $branchThumbnail = $this->request->getFile('branchThumbnail');
                $branchThumbnailName = $branchThumbnail->getRandomName();
                $branchThumbnail->move(ROOTPATH . 'public/uploads/branch_thumbnails', $branchThumbnailName);
                $formData['branch_thumbnail'] = $branchThumbnailName;
            }

            $file = $this->request->getFile('branchBanner1');
            if ($file->isValid()) {

                /* upload branch banner 1*/
                $branchBanner1 = $this->request->getFile('branchBanner1');
                $branchBanner1Name = $branchBanner1->getRandomName();
                $branchBanner1->move(ROOTPATH . 'public/uploads/branch_banners', $branchBanner1Name);
                $formData['branch_banner1'] = $branchBanner1Name;
            }

            $file = $this->request->getFile('branchBanner2');
            if ($file->isValid()) {
                /* upload branch banner 2*/
                $branchBanner2 = $this->request->getFile('branchBanner2');
                $branchBanner2Name = $branchBanner2->getRandomName();
                $branchBanner2->move(ROOTPATH . 'public/uploads/branch_banners', $branchBanner2Name);
                $formData['branch_banner2'] = $branchBanner2Name;
            }

            $file = $this->request->getFile('branchBanner3');
            if ($file->isValid()) {
                /* upload branch banner 3*/
                $branchBanner3 = $this->request->getFile('branchBanner3');
                $branchBanner3Name = $branchBanner3->getRandomName();
                $branchBanner3->move(ROOTPATH . 'public/uploads/branch_banners', $branchBanner3Name);
                $formData['branch_banner3'] = $branchBanner3Name;
            }

            // Update the data into the database table
            $result = $this->BranchModel->updateData($this->request->getPost('branchId'), $formData);
            ...
            // Return a success JSON response
            return $this->response->setJSON(['status' => 'success', 'success' => true, 'message' => 'Branch Details updated successfully.']);
        } catch (\Exception $e) {
            // An error occurred, rollback the transaction
            $db->transRollback();
            ...
            // Error handling and logging
            $logger = \Config\Services::logger();
            $logger->error('Error occurred while updating Branch Details: ' . $e->getMessage());
    ...
    public function change_branch_status($statusChange) {
        try {
            // Get the form input values
            $Id = $this->request->getPost('id');
            $updated_by = $this->session->get('adminData.loggedUserInfo.id');
            ...
            if ($statusChange == "enable") {
                $is_active = '1';
                $textMsg = "Branch Activated successfully.";
            } else {
                $is_active = '2';
                $textMsg = "Branch DeActivated successfully.";
            }
            $formData = [
                'is_active'  => $is_active,
                'updated_at' => $updated_datetime
            ...
            // Update the data into the database table
            $result = $this->BranchModel->updateData($Id, $formData);
            ...
            // Error handling and logging
            $logger = \Config\Services::logger();
            $logger->error('Error occurred while removing Branch Details: ' . $e->getMessage());
